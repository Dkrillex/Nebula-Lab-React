...
const InviteRecord: React.FC<InviteRecordProps> = ({ t }) => {
  ...
  const fetchInviteList = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error.message || (t?.invite?.fetchFailed || '获取邀请记录失败'));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-border p-6">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-foreground">{t?.invite?.title || '推广邀请记录'}</h3>
        <p className="text-sm text-muted mt-1">{t?.invite?.subtitle || '查看通过您的邀请码注册的用户记录'}</p>
      </div>
        ...
        <>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-3 px-4 text-sm font-medium text-foreground">{t?.invite?.tableHeaders?.inviteCode || '邀请码'}</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-foreground">{t?.invite?.tableHeaders?.invitedUser || '受邀用户账号'}</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-foreground">{t?.invite?.tableHeaders?.registerTime || '注册时间'}</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
          ...
          <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
            <div className="text-sm text-muted">
              {t?.invite?.pagination?.total?.replace('{total}', String(total)) || `共 ${total} 条记录`}{totalPages > 0 && (t?.invite?.pagination?.page?.replace('{current}', String(pageNum)).replace('{total}', String(totalPages)) || `，第 ${pageNum} / ${totalPages} 页`)}
            </div>
            {totalPages > 0 && (
              <div className="flex items-center gap-2">
                <button
                ...
                >
                  {t?.invite?.pagination?.firstPage || '首页'}
                </button>
                <button
                ...
                >
                  {t?.invite?.pagination?.prevPage || '上一页'}
                </button>
                ...
                <button
                  ...
                  className="px-4 py-2 border border-border rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {t?.invite?.pagination?.nextPage || '下一页'}
                </button>
                <button
                  ...
                  className="px-3 py-2 border border-border rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                >
                  {t?.invite?.pagination?.lastPage || '末页'}
                </button>
              </div>
            )}
          </div>
